...
{
public:
    AbstractPidHandlerPrivate(AbstractPidHandler *parent) :
        p(parent),
        ecc(INVALID_CC),
        attached(false),
        parser(p),
        section(0)
    {
        QObject::connect(&parser, SIGNAL(complete()),
                         p, SLOT(tableComplete()));
        QObject::connect(p, SIGNAL(discontinuity()),
                         &parser, SLOT(reinit()));
    }

    AbstractPidHandler *p;
    quint8 ecc;
    bool attached;
    TableParser parser;
    Section *section;
...
AbstractPidHandler::~AbstractPidHandler()
{
    delete d;
}

void AbstractPidHandler::push(const QByteArray &packet)
{
    if (d->attached) {
...
void AbstractPidHandler::attach()
{
    d->attached = true;
}

void AbstractPidHandler::dettach()
{
    d->attached = false;
}

bool AbstractPidHandler::isAttached()
{
    return d->attached;
}

bool AbstractPidHandler::checkContinuity(const QByteArray &packet)
{
    ...
    if (d->ecc == INVALID_CC) { // Conintuity counter has not been yet initialized
        ...
        d->ecc = ++d->ecc & 0x0F; // Compute next continuity counter
        if (d->ecc != ccc) { // Check if discontinuity
...
void AbstractPidHandler::parse(const QByteArray &packet)
{
    ...
    while (available) {
        ...
        if (available >= d->section->need) {
            ...
            d->section->data += QByteArray((const char *)p, (int)available);
            d->section->need -= available;
            available = 0;
        }
    }

//    // If pusi is set then it shall the beginning of a table
//    quint8 available = 188 + data - payload;
//    if (pusi) {
//        d->parser.parse(payload, available);
//    } else if (d->parser.valid) {
//        d->parser.append(payload, available);
    //    }
}

bool AbstractPidHandler::checkCRC()
{
    int index = 0;
//    const quint8* data = section.constData();
    quint32 tmp = 0xffffffff;

    while (index < d->section->data.size()) {
        quint32 a = tmp << 8;
        quint32 b = tmp >> 24;
        tmp = a ^ crc32Table[b] ^ d->section->data[index++];
...
void AbstractPidHandler::deleteCurrentSection()
{
    if (d->section) {
        delete d->section;
        d->section = 0;
...
void AbstractPidHandler::tableComplete()
{
    parseTable(d->section);
//    d->parser.reinit();
}
